...
def testCount():
    ...
    deleteTournaments()
    deletePlayers()
    curT = registerTournament("MyTournament")
    c = countPlayers(curT)
    if c == '0':
        raise TypeError(
    ...
    registerPlayer("Jace Beleren",curT)
    c = countPlayers(curT)
    if c != 2:
        raise ValueError(
...
def testStandingsBeforeMatches():
    ...
    if len(standings[0]) != 5: # --> 5 because of the OMW (Opponent Match Wins)
...
def testPairings():
    ...
    registerPlayer("Rarity",curT)
    registerPlayer("Rainbow Dash",curT)
    registerPlayer("Princess Celestia",curT)
    registerPlayer("Princess Luna",curT)
    standings = playerStandings(curT)
    ...
    reportMatch(curT, id1, id2, False)
    reportMatch(curT, id3, id4, False)
    reportMatch(curT, id5, id6, False)
    reportMatch(curT, id7, id8, False)
    pairings = swissPairings(curT)
    if len(pairings) != 4:
        raise ValueError(
            "For eight players, swissPairings should return 4 pairs. Got {pairs}".format(pairs=len(pairings)))
